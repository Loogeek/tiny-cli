...
import { isRecord, addRecord, getTargetFileImagesCount } from '../src/utils';
describe('should', () => {
  it('exported', () => {
    expect(1).toEqual(1);
  });
...
it.skip('test writeJsonFile', () => {
  ...
  addRecord(cwd(), pathDir);
});

it('get extname', () => {
  const pathDir = path.resolve(cwd(), './src/d.text');
...
it('is record', async () => {
  ...
  const recorded = await isRecord(cwd(), pathDir);
...
it('png jpeg jpg pattern', async () => {
  const targetDir = './src/';
  const len = await getTargetFileImagesCount(targetDir);
